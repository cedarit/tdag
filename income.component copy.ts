...
export class IncomeComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    // localStorage memberProfile
    const mStoredData = localStorage.getItem("memberProfile"); 
    const mData = mStoredData ? JSON.parse(mStoredData) : null;
    ...
    // this is DisplayName
    if (mData && mData.length > 0 && mData[0].displayName) {
      this.psDisplayName = mData[0].displayName;
    } else {
      // sometimes due to direct loading to this page psData is not loaded
      //call a service to set the psData
